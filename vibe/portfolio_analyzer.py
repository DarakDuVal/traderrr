"""
portfolio_analyzer.py - Advanced portfolio analysis and risk management
"""
...
class PortfolioAnalyzer:
    ...
    def analyze_portfolio(self, 
                         ...
                         benchmark_ticker: str = 'SPY') -> PortfolioMetrics:
        """
    ...
    def calculate_position_risks(self,
                               portfolio_data: Dict[str, pd.DataFrame],
                               portfolio_weights: Dict[str, float],
                               portfolio_value: float) -> List[PositionRisk]:
    ...
    def optimize_portfolio(self,
                          ...
                          risk_tolerance: float = 0.15) -> Dict[str, float]:
        """
        ...
        try:
            ...
            if result.success:
                ...
                optimized_weights = {
                    ticker: weight for ticker, weight in optimized_weights.items()
                    if weight > 0.01
                }
    ...
    def calculate_correlation_matrix(self,
                                   portfolio_data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    def _calculate_portfolio_returns(self,
        ...
        # Normalize weights
        normalized_weights = {ticker: weights[ticker] / total_weight for ticker in available_tickers}
    ...
    def _analyze_sector_concentration(self, portfolio_weights: Dict[str, float]) -> Dict:
        ...
        sector_mapping = {
            ...
            'PG': 'Consumer Staples', 'JNJ': 'Healthcare', 'KO': 'Consumer Staples',
            'VTI': 'Diversified', 'SPY': 'Diversified',
            'SIEGY': 'Industrial', 'VWAGY': 'Auto', 'SYIEY': 'Consumer Goods', 'BYDDY': 'Auto',
            'QTUM': 'Technology', 'QBTS': 'Technology'
        }
    ...
    def _run_stress_tests(self,
        ...
        try:
            ...
            scenarios = {
                ...
                'correlation_breakdown': portfolio_returns.std() * 2  # 2-sigma with correlation increase
            }
    ...
    def _generate_risk_recommendations(self,
        ...
        if portfolio_metrics.volatility > 0.25:
            recommendations.append("Portfolio volatility is high. Consider reducing position sizes or adding defensive assets.")
        
        # Sharpe ratio check
        if portfolio_metrics.sharpe_ratio < 0.5:
            recommendations.append("Sharpe ratio is low. Review asset selection and consider rebalancing.")
        
        # Maximum drawdown check
        if portfolio_metrics.max_drawdown < -0.20:
            recommendations.append("Maximum drawdown exceeds 20%. Implement stronger risk controls.")
        
        # Concentration risk
        high_concentration_positions = [pos for pos in position_risks if pos.concentration_risk > 0.6]
        if high_concentration_positions:
            tickers = [pos.ticker for pos in high_concentration_positions]
            recommendations.append(f"High concentration risk in: {', '.join(tickers)}. Consider reducing position sizes.")
        ...
        if low_liquidity_positions:
            ...
            recommendations.append(f"Low liquidity in: {', '.join(tickers)}. Monitor for exit opportunities.")
        
        # Correlation risk
        if not correlation_matrix.empty:
            avg_correlation = correlation_matrix.values[np.triu_indices_from(correlation_matrix.values, k=1)].mean()
            if avg_correlation > 0.6:
                recommendations.append("High average correlation detected. Increase diversification across uncorrelated assets.")
...
if __name__ == "__main__":
    ...
    if portfolio_data:
        print(f"\nAnalyzing portfolio with {len(portfolio_data)} assets...")
        ...
        # Position risks
        position_risks = analyzer.calculate_position_risks(portfolio_data, weights, portfolio_value)
        print(f"\nTop Risk Contributors:")
        for pos in position_risks[:3]:
            print(f"  {pos.ticker}: {pos.contribution_to_risk:.4f} (Weight: {pos.weight:.1%})")
        
        # Correlation matrix
        corr_matrix = analyzer.calculate_correlation_matrix(portfolio_data)
        print(f"\nAverage Correlation: {corr_matrix.values[np.triu_indices_from(corr_matrix.values, k=1)].mean():.2f}")
        
        # Portfolio optimization
        print(f"\nOptimizing portfolio...")
        optimized_weights = analyzer.optimize_portfolio(portfolio_data, weights, risk_tolerance=0.20)
        ...
        risk_report = analyzer.generate_risk_report(portfolio_data, weights, portfolio_value)
