"""
signal_generator.py - Advanced trading signal generation system
...
class SignalGenerator:
    ...
    def generate_portfolio_signals(self, 
                                 portfolio_data: Dict[str, pd.DataFrame]) -> List[TradingSignal]:
        """
    ...
    def _momentum_strategy(self, 
                          data: pd.DataFrame, 
                          indicators: Dict[str, float], 
                          regime: MarketRegime) -> Optional[Dict]:
    ...
    def _calculate_confidence(self, 
        ...
        if regime in [MarketRegime.TRENDING_UP, MarketRegime.TRENDING_DOWN]:
            if signal_type in [SignalType.BUY, SignalType.STRONG_BUY] and regime == MarketRegime.TRENDING_UP:
                confidence += 0.2
            elif signal_type in [SignalType.SELL, SignalType.STRONG_SELL] and regime == MarketRegime.TRENDING_DOWN:
                confidence += 0.2
...
if __name__ == "__main__":
    ...
    if signal:
        print(f"Signal Generated for TEST:")
        ...
        print("No signal generated for TEST")
    
    # Test portfolio signals
    portfolio_data = {
        'AAPL': test_data.copy(),
        'MSFT': test_data.copy() * 1.1,  # Different price level
        'GOOGL': test_data.copy() * 0.9
    }
    
    portfolio_signals = signal_gen.generate_portfolio_signals(portfolio_data)
    summary = signal_gen.get_signal_summary(portfolio_signals)
    ...
    if summary['top_picks']:
        ...
        for pick in summary['top_picks']:
            print(f"  {pick['ticker']}: {pick['signal']} (Confidence: {pick['confidence']:.2f})")
