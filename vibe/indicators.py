...
if __name__ == "__main__":
    ...
    close_prices = trend + noise + np.cumsum(np.random.normal(0, 0.5, 252))
    
    # Create OHLC data
    close = pd.Series(close_prices, index=dates)
    high = close + np.random.uniform(0, 2, 252)
    low = close - np.random.uniform(0, 2, 252)
    open_prices = close.shift(1) + np.random.normal(0, 0.5, 252)
    ...
    print(f"RSI: {ti.rsi(close).iloc[-1]:.2f}")
    ...
    print(f"BB Position: {((close.iloc[-1] - lower.iloc[-1]) / (upper.iloc[-1] - lower.iloc[-1]) * 100):.1f}%")
    
    # Test regime detection
    regime = MarketRegimeDetector()
    hurst = regime.hurst_exponent(close)
    trend_str = regime.trend_strength(close)
    vol_regime = regime.volatility_regime(close)
    ...
    momentum_score = adv.composite_momentum(data)
    mr_score = adv.mean_reversion_score(data)
    
    print(f"\nAdvanced Signals:")
    print(f"Momentum Score: {momentum_score.iloc[-1]:.1f}")
    print(f"Mean Reversion Score: {mr_score.iloc[-1]:.1f}")
